...
export const selectCompletedUserPlans = createSelector(
  [(state) => state.plan, (state, userId) => userId],
  (plan, userId) =>
    plan.filter((p) => p.userId === userId && p.isFinalized === true)
)

export const selectNonCompletedUserPlans = createSelector(
  [(state) => state.plan, (state, userId) => userId],
  (plan, userId) =>
    plan.filter((p) => p.userId === userId && p.isFinalized === false)
)

export const { planAdded, planDeleted, finalizePlan, deletePlan } =
  planSlice.actions
